...
public class Plateau {
	...
	/**
	 * Crï¿½ation du plateau avec <b><i>SIZE</i></b> cases en <i> LONGUEUR et LARGEUR</i> 
	...
	/**
	 * Incrï¿½mentation du tableau des anciens dï¿½placements du monstres 
	 * @param m - Monstre
	 */
	...
	/**
	 * Le chasseur rï¿½vï¿½le la case en X,Y du plateau
	 * @param x int
	 * @param y int 
	 * @return true: si le chasseur a trouvï¿½ le monstre sinon false
	 */
	public boolean reveal(int x, int y, Monstre m) {
		if(m.getX() == x && m.getY() == y) return true;
		if(monstreAnciennePosition[x][y] >= 0) {
			System.out.println("\nLe monstre est passï¿½ par la il y a : " + monstreAnciennePosition[x][y] + " tours !");
	...
	/**
	 * Renvoie vrai ou faux si la case a ï¿½tï¿½ explorï¿½ en X,Y
	 * @param x int
	 * @param y int
	 * @return true si la case ï¿½ dï¿½jï¿½ ï¿½tï¿½ explorï¿½ par le monstre sinon false
	 */
